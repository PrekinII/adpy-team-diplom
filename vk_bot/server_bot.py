...
class Server_bot:
    ...
    def send_msg(self, send_id, message, keyboard=None):  # send_id он же peer_id, он же id пользователя, которому отвечаем
        if keyboard:
    ...
    def find_pair(self):  # При получении сообщения от пользователя предлагаем найти пару
        for event in self.long_poll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                request = event.obj.message["text"]
                if request == 'Привет':
                    self.get_user_info(event.obj.message["from_id"])
                    self.send_msg(event.obj.message["peer_id"],
                                  message="Хелоу")
                    keyboard_oauth = self.user_token_button(self.api_id)
                    self.send_msg(event.obj.message["peer_id"],  # Отправляем пользователю кнопку на токен
    ...
    def user_token_button(self, group_id):  # Создаем кнопку запроса токена
        ...
        params = {'client_id': group_id, 'redirect_uri': 'https://oauth.vk.com/authorize', 'display': 'page',
    ...
    def show_friends_button(self):  # Кнопки для фото и прочего
        ...
        keyboard_2.add_button(label="Следующий", color=VkKeyboardColor.PRIMARY, payload={"type": "show_next_user"})
        keyboard_2.add_button(label='В избранные', color=VkKeyboardColor.SECONDARY)
        keyboard_2.add_line()
        keyboard_2.add_button(label='Показать избранных', color=VkKeyboardColor.POSITIVE)
        return keyboard_2
        ...
        for event in self.long_poll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if request == 'Старт':
                    ...
                    self.send_msg(event.obj.message["peer_id"],  # Отправляем кнопки
        ...
        for event in self.long_poll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if request == 'Следующий':
                    self.send_msg(event.obj.message["peer_id"],  # Сюда должна прилететь информация (Возможно фотки придется отправить дополнительным письмом
                                  message="-имя, фамилия\n-ссылка на профиль-\n-три фотографии"
                                  )
                elif request == 'В избранные':
                    self.send_msg(event.obj.message["peer_id"],  # Добавляем в избранные
                                  message="Добавлен в избранные"
                                  )
                elif request == 'Показать избранных':
                    self.send_msg(event.obj.message["peer_id"],  # Отправляем список избранных
